...
service.interceptors.request.use(config => {
  ...
  if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put')) {
    ...
    if (requestSize >= limitSize) {
      console.warn(`[${config.url}]: ` + 'The request data size exceeds the allowed 5M limit and cannot perform duplicate submission verification.')
      return config;
    }
...
service.interceptors.response.use(res => {
    ...
    if (code === 401) {
      if (!isRelogin.show) {
        isRelogin.show = true;
        MessageBox.confirm('Login status has expired. You may stay on this page or log in again.', 'System Prompt', { confirmButtonText: 'Re-login', cancelButtonText: 'Cancel', type: 'warning' }).then(() => {
