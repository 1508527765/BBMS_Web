      ...
      <div class="toolbar" style="margin-left: auto">
        ...
        >
          <el-upload
            ...
            :action="upload.url + '?settingType=run' + '&lang=' + currentLocale"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        const res = await fetchConfigList("run", this.currentPage);
    ...
    importTemplate() {
      this.download(
        ...
        {},
        `run_template_${new Date().getTime()}.xlsx`
      );
    },
    /** 文件上传中处理 */
    ...
    handleExport() {
      this.download(
        "control-strategy/export-configs/run/" + this.currentLocale,
        {},
        `run_${new Date().getTime()}.xlsx`
      );
    },
  },
  async mounted() {
