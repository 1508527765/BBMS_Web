      ...
      <div class="toolbar" style="margin-left: auto">
        ...
        >
          <el-upload
            ...
            :action="
              upload.url + '?settingType=protect' + '&lang=' + currentLocale
            "
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        const res = await fetchConfigList("protect", this.currentPage);
    ...
    importTemplate() {
      this.download(
        ...
        {},
        `protect_template_${new Date().getTime()}.xlsx`
      );
    },
    /** 文件上传中处理 */
    ...
    handleExport() {
      this.download(
        "control-strategy/export-configs/protect/" + this.currentLocale,
        {},
        `protect_${new Date().getTime()}.xlsx`
      );
    },
  },
  async mounted() {
