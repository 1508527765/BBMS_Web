...
class UserService:
    ...
    async def get_user_list_services(
        cls, query_db: Session, query_object: UserPageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        获取用户列表信息service
    ...
    async def check_user_allowed_services(cls, check_user: UserModel):
        ...
        """
        if check_user.admin:
            raise ServiceException(message='Operation on super admin user is not allowed')
        else:
            return CrudResponseModel(is_success=True, message='Validation passed')

    @classmethod
    async def check_user_data_scope_services(cls, query_db: Session, user_id: int, data_scope_sql: str):
        """
        校验用户数据权限service
        ...
        if users:
            ...
            raise ServiceException(message='No permission to access user data')

    @classmethod
    async def check_user_name_unique_services(cls, query_db: Session, page_object: UserModel):
        """
        校验用户名是否唯一service
    ...
    @classmethod
    async def check_phonenumber_unique_services(cls, query_db: Session, page_object: UserModel):
        """
        校验用户手机号是否唯一service
        ...
        """
        user_id = -1 if page_object.user_id is None else page_object.user_id
        user = UserDao.get_user_by_info(query_db, UserModel(phonenumber=page_object.phonenumber))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_email_unique_services(cls, query_db: Session, page_object: UserModel):
        """
        校验用户邮箱是否唯一service
        ...
        """
        user_id = -1 if page_object.user_id is None else page_object.user_id
        user = UserDao.get_user_by_info(query_db, UserModel(email=page_object.email))
        if user and user.user_id != user_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def add_user_services(cls, query_db: Session, page_object: AddUserModel):
        """
        新增用户信息service
        ...
        if not cls.check_user_name_unique_services(query_db, page_object):
            raise ServiceException(message=f"Failed to add user {page_object.user_name}, login account already exists")
        elif page_object.phonenumber and not cls.check_phonenumber_unique_services(query_db, page_object):
            raise ServiceException(message=f"Failed to add user {page_object.user_name}, phone number already exists")
        elif page_object.email and not cls.check_email_unique_services(query_db, page_object):
            raise ServiceException(message=f"Failed to add user {page_object.user_name}, email already exists")
        else:
            try:
    ...
    @classmethod
    async def edit_user_services(cls, query_db: Session, page_object: EditUserModel):
        """
        编辑用户信息service
        ...
        if user_info.data and user_info.data.user_id:
            if page_object.type != 'status' and page_object.type != 'avatar' and page_object.type != 'pwd':
                if not await cls.check_user_name_unique_services(query_db, page_object):
                    raise ServiceException(message=f"Failed to edit user {page_object.user_name}, login account already exists")
                elif page_object.phonenumber and not await cls.check_phonenumber_unique_services(query_db, page_object):
                    raise ServiceException(message=f"Failed to edit user {page_object.user_name}, phone number already exists")
                elif page_object.email and not await cls.check_email_unique_services(query_db, page_object):
                    raise ServiceException(message=f"Failed to edit user {page_object.user_name}, email already exists")
            try:
            ...
            raise ServiceException(message='User does not exist')

    @classmethod
    async def delete_user_services(cls, query_db: Session, page_object: DeleteUserModel):
        """
        删除用户信息service
        ...
        if page_object.user_ids:
            ...
            raise ServiceException(message='User ID is empty')

    @classmethod
    async def user_detail_services(cls, query_db: Session, user_id: Union[int, str]):
        """
        获取用户详细信息service
    ...
    @classmethod
    async def user_profile_services(cls, query_db: Session, user_id: int):
        """
        获取用户个人详细信息service
    ...
    @classmethod
    async def reset_user_services(cls, query_db: Session, page_object: ResetUserModel):
        """
        重置用户密码service
        ...
        if page_object.old_password:
            ...
            if not PwdUtil.verify_password(page_object.old_password, user.password):
                raise ServiceException(message='Failed to change password: incorrect old password')
            elif PwdUtil.verify_password(page_object.password, user.password):
                raise ServiceException(message='New password cannot be the same as old password')
            else:
        ...
        try:
            ...
            query_db.commit()
            return CrudResponseModel(is_success=True, message='Password reset successful')
        except Exception as e:
            query_db.rollback()
            raise e
            ...
            for index, row in df.iterrows():
                ...
                add_user = UserModel(
                    ...
                    password=PwdUtil.get_password_hash(
                        ConfigService.query_config_list_from_cache_services(
                ...
                if user_info:
                    if update_support:
                        ...
                        cls.check_user_allowed_services(edit_user_model)
                        if not current_user.user.admin:
                            cls.check_user_data_scope_services(
                                query_db, edit_user_model.user_id, user_data_scope_sql
                            )
                            DeptService.check_dept_data_scope_services(
                        ...
                        add_error_result.append(f"{count}.User account {row['user_name']} already exists")
                else:
    ...
    @classmethod
    async def get_user_role_allocated_list_services(cls, query_db: Session, page_object: UserRoleQueryModel):
        """
        根据用户id获取已分配角色列表
    ...
    @classmethod
    async def add_user_role_services(cls, query_db: Session, page_object: CrudUserRoleModel):
        """
        新增用户关联角色信息service
        ...
        if page_object.user_id and page_object.role_ids:
            ...
            try:
                ...
                query_db.commit()
                return CrudResponseModel(is_success=True, message='Assigned successfully')
            except Exception as e:
                query_db.rollback()
                raise e
            ...
            try:
                for user_id in user_id_list:
                    user_role = cls.detail_user_role_services(
            ...
            raise ServiceException(message='Conditions not met for adding')

    @classmethod
    async def delete_user_role_services(cls, query_db: Session, page_object: CrudUserRoleModel):
        """
        删除用户关联角色信息service
        ...
        if (page_object.user_id and page_object.role_id) or (page_object.user_ids and page_object.role_id):
            if page_object.user_id and page_object.role_id:
                ...
                raise ServiceException(message='Conditions not met for deletion')
        else:
            raise ServiceException(message='User-role association information is empty')

    @classmethod
    async def detail_user_role_services(cls, query_db: Session, page_object: UserRoleModel):
        """
        获取用户关联角色详细信息service
