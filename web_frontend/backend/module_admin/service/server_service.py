...
class ServerService:
    ...
    @staticmethod
    async def get_server_monitor_info(lang: str = 'cn'):
        # CPU信息
        # 获取CPU总核心数
        ...
        # 根据语言参数返回不同的文本
        if lang == 'en':
            run_time = f'{days} days {hours} hours {minutes} minutes'
        else:
        ...
        for i in io:
            ...
            # 根据语言参数返回不同的文本
            if lang == 'en':
                type_name = f"Local Fixed Disk ({i.mountpoint})"
            else:
                type_name = f"本地固定磁盘（{i.mountpoint}）"

            disk_data = SysFiles(
                dirName=i.mountpoint,
