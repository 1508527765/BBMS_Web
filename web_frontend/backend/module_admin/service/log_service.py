...
class OperationLogService:
    ...
    async def get_operation_log_list_services(
        cls, query_db: Session, query_object: OperLogPageQueryModel, is_page: bool = False
    ):
        """
        获取操作日志列表信息service
    ...
    @classmethod
    async def add_operation_log_services(cls, query_db: Session, page_object: OperLogModel):
        """
        新增操作日志service
    ...
    @classmethod
    async def delete_operation_log_services(cls, query_db: Session, page_object: DeleteOperLogModel):
        """
        删除操作日志信息service
        ...
        if page_object.oper_ids:
            ...
            raise ServiceException(message='The operation log id is empty')

    @classmethod
    async def clear_operation_log_services(cls, query_db: Session):
        """
        清除操作日志信息service
        ...
        try:
            ...
            query_db.commit()
            return CrudResponseModel(is_success=True, message='Clear successful')
        except Exception as e:
            query_db.rollback()
            raise e
    ...
    async def export_operation_log_list_services(cls, request: Request, operation_log_list: List):
        ...
        operation_type_list = DictDataService.query_dict_data_list_from_cache_services(dict_type='sys_oper_type')
...
class LoginLogService:
    ...
    async def get_login_log_list_services(
        cls, query_db: Session, query_object: LoginLogPageQueryModel, is_page: bool = False
    ):
        """
        获取登录日志列表信息service
    ...
    @classmethod
    async def add_login_log_services(cls, query_db: Session, page_object: LogininforModel):
        """
        新增登录日志service
    ...
    @classmethod
    async def delete_login_log_services(cls, query_db: Session, page_object: DeleteLoginLogModel):
        """
        删除操作日志信息service
        ...
        if page_object.info_ids:
            ...
            raise ServiceException(message='The login log id is empty')

    @classmethod
    async def clear_login_log_services(cls, query_db: Session):
        """
        清除操作日志信息service
    ...
    async def unlock_user_services(cls, request: Request, unlock_user: UnlockUser):
        locked_user = request.app.state.redis.get(f'account_lock:{unlock_user.user_name}')
        if locked_user:
            request.app.state.redis.delete(f'account_lock:{unlock_user.user_name}')
            return CrudResponseModel(is_success=True, message='Unlock successful')
        else:
            raise ServiceException(message='The user is not locked')
