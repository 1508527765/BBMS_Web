...
class RoleService:
    ...
    @classmethod
    async def get_role_select_option_services(cls, query_db: Session):
        """
        获取角色列表不分页信息service
    ...
    @classmethod
    async def get_role_dept_tree_services(cls, query_db: Session, role_id: int):
        """
        ...
        """
        role = cls.role_detail_services(query_db, role_id)
        role_dept_list = RoleDao.get_role_dept_dao(query_db, role)
    ...
    async def get_role_list_services(
        cls, query_db: Session, query_object: RolePageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        获取角色列表信息service
    ...
    async def check_role_allowed_services(cls, check_role: RoleModel):
        ...
        """
        if check_role.admin:
            raise ServiceException(message='Operation not allowed for super administrator role')
        else:
            return CrudResponseModel(is_success=True, message='Validation passed')

    @classmethod
    async def check_role_data_scope_services(cls, query_db: Session, role_ids: str, data_scope_sql: str):
        """
        校验角色是否有数据权限service
        ...
        if role_id_list:
            for role_id in role_id_list:
                roles = RoleDao.get_role_list(
                    query_db, RolePageQueryModel(roleId=int(role_id)), data_scope_sql, is_page=False
                )
                if roles:
                    continue
                else:
                    raise ServiceException(message='No permission to access role data')

    @classmethod
    async def check_role_name_unique_services(cls, query_db: Session, page_object: RoleModel):
        """
        校验角色名称是否唯一service
        ...
        role = RoleDao.get_role_by_info(query_db, RoleModel(roleName=page_object.role_name))
        if role and role.role_id != role_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def check_role_key_unique_services(cls, query_db: Session, page_object: RoleModel):
        """
        校验角色权限字符是否唯一service
        ...
        """
        role_id = -1 if page_object.role_id is None else page_object.role_id
        role = RoleDao.get_role_by_info(query_db, RoleModel(roleKey=page_object.role_key))
        if role and role.role_id != role_id:
            return CommonConstant.NOT_UNIQUE
        return CommonConstant.UNIQUE

    @classmethod
    async def add_role_services(cls, query_db: Session, page_object: AddRoleModel):
        """
        新增角色信息service
        ...
        if not cls.check_role_name_unique_services(query_db, page_object):
            raise ServiceException(message=f'Failed to add role {page_object.role_name}: Role name already exists')
        elif not cls.check_role_key_unique_services(query_db, page_object):
            raise ServiceException(message=f'Failed to add role {page_object.role_name}: Role key already exists')
        else:
            try:
    ...
    @classmethod
    async def edit_role_services(cls, query_db: Session, page_object: AddRoleModel):
        """
        编辑角色信息service
        ...
        if role_info:
            if page_object.type != 'status':
                if not cls.check_role_name_unique_services(query_db, page_object):
                    raise ServiceException(message=f'Failed to edit role {page_object.role_name}: Role name already exists')
                elif not cls.check_role_key_unique_services(query_db, page_object):
                    raise ServiceException(message=f'Failed to edit role {page_object.role_name}: Role key already exists')
            try:
            ...
            raise ServiceException(message='Role does not exist')

    @classmethod
    async def role_datascope_services(cls, query_db: Session, page_object: AddRoleModel):
        """
        分配角色数据权限service
        ...
        role_info = cls.role_detail_services(query_db, page_object.role_id)
            ...
            try:
                ...
                if page_object.dept_ids and page_object.data_scope == '2':
                    for dept in page_object.dept_ids:
                        RoleDao.add_role_dept_dao(
                            query_db, RoleDeptModel(roleId=page_object.role_id, deptId=dept)
                        )
                query_db.commit()
                return CrudResponseModel(is_success=True, message='Data scope assigned successfully')
            except Exception as e:
                query_db.rollback()
                raise e
        else:
    ...
    @classmethod
    async def delete_role_services(cls, query_db: Session, page_object: DeleteRoleModel):
        """
        删除角色信息service
        ...
        if page_object.role_ids:
            ...
            try:
                for role_id in role_id_list:
                    role = cls.role_detail_services(query_db, int(role_id))
                    if (RoleDao.count_user_role_dao(query_db, int(role_id))) > 0:
                        raise ServiceException(message=f'Role {role.role_name} is assigned, cannot delete')
                    role_id_dict = dict(
                    ...
                    RoleDao.delete_role_dept_dao(query_db, RoleDeptModel(**role_id_dict))
            ...
            raise ServiceException(message='Role ID is empty')

    @classmethod
    async def role_detail_services(cls, query_db: Session, role_id: int):
        """
        获取角色详细信息service
    ...
    async def get_role_user_allocated_list_services(
        cls, query_db: Session, page_object: UserRolePageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        根据角色id获取已分配用户列表
        ...
        :return: 已分配用户列表
        """
        query_user_list = UserDao.get_user_role_allocated_list_by_role_id(
    ...
    async def get_role_user_unallocated_list_services(
        ...
        :return: 未分配用户列表
        """
        query_user_list = UserDao.get_user_role_unallocated_list_by_role_id(
            query_db, page_object, data_scope_sql, is_page
        )
