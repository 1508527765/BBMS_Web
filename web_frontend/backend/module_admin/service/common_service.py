...
class CommonService:
    ...
    async def upload_service(cls, request: Request, file: UploadFile):
        ...
        if not UploadUtil.check_file_extension(file):
            raise ServiceException(message='Invalid file type')
        else:
    ...
    async def download_services(cls, background_tasks: BackgroundTasks, file_name, delete: bool):
        ...
        if '..' in file_name:
            raise ServiceException(message='Invalid file name')
        elif not UploadUtil.check_file_exists(filepath):
            raise ServiceException(message='File does not exist')
        else:
            if delete:
                background_tasks.add_task(UploadUtil.delete_file, filepath)
            return CrudResponseModel(is_success=True, result=UploadUtil.generate_file(filepath), message='Download succeeded')
