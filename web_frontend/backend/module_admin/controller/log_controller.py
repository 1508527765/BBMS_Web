...
async def delete_system_operation_log(request: Request, oper_ids: str, query_db: Session = Depends(get_db)):
    ...
    delete_operation_log_result = OperationLogService.delete_operation_log_services(
...
async def export_system_operation_log_list(
    ...
    operation_log_export_result = OperationLogService.export_operation_log_list_services(
...
async def unlock_system_user(request: Request, user_name: str, query_db: Session = Depends(get_db)):
    unlock_user = UnlockUser(userName=user_name)
    unlock_user_result = LoginLogService.unlock_user_services(request, unlock_user)
