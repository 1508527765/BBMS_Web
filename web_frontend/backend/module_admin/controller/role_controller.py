...
async def edit_system_role(
    ...
    RoleService.check_role_allowed_services(edit_role)
    if not current_user.user.admin:
        RoleService.check_role_data_scope_services(query_db, str(edit_role.role_id), data_scope_sql)
...
async def edit_system_role_datascope(
    ...
    RoleService.check_role_allowed_services(role_data_scope)
    if not current_user.user.admin:
        RoleService.check_role_data_scope_services(query_db, str(role_data_scope.role_id), data_scope_sql)
    edit_role = AddRoleModel(
    ...
    role_data_scope_result = RoleService.role_datascope_services(query_db, edit_role)
...
async def delete_system_role(
    ...
    if role_id_list:
        for role_id in role_id_list:
            RoleService.check_role_allowed_services(RoleModel(roleId=int(role_id)))
            if not current_user.user.admin:
                RoleService.check_role_data_scope_services(query_db, role_id, data_scope_sql)
...
async def reset_system_role_status(
    ...
    RoleService.check_role_allowed_services(change_role)
    if not current_user.user.admin:
        RoleService.check_role_data_scope_services(query_db, str(change_role.role_id), data_scope_sql)
    edit_role = AddRoleModel(
...
async def get_system_allocated_user_list(
    ...
    role_user_allocated_page_query_result = RoleService.get_role_user_allocated_list_services(
...
async def get_system_unallocated_user_list(
    ...
    role_user_unallocated_page_query_result = RoleService.get_role_user_unallocated_list_services(
        query_db, user_role, data_scope_sql, is_page=True
    )
    logger.info('获取成功')
...
async def add_system_role_user(
    ...
    if not current_user.user.admin:
        RoleService.check_role_data_scope_services(query_db, str(add_role_user.role_id), data_scope_sql)
...
async def cancel_system_role_user(
    request: Request, cancel_user_role: CrudUserRoleModel, query_db: Session = Depends(get_db)
):
