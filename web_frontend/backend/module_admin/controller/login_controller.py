...
async def login(
    ...
    try:
        # Keep login available even if profile/status update fails
        # (e.g. environment-specific DB/schema differences).
        ...
        logger.warning(f'Login status update skipped: {e}')
    logger.info('登录成功')
    ...
    return ResponseUtil.success(msg='Login successful', dict_content={'token': access_token})
...
async def get_login_user_routers(
    ...
    logger.info('获取成功')
    user_routers = await LoginService.get_current_user_routers(current_user.user.user_id, lang, query_db)
...
async def register_user(request: Request, user_register: UserRegister, query_db: Session = Depends(get_db)):
    user_register_result = LoginService.register_user_services(request, query_db, user_register)
...
#     try:
#         sms_result = LoginService.get_sms_code_services(request, query_db, user)
...
#     try:
#         forget_user_result = LoginService.forget_user_services(request, query_db, forget_user)
...
async def logout(request: Request, token: Optional[str] = Depends(oauth2_scheme)):
    ...
    return ResponseUtil.success(msg='Logout successful')
