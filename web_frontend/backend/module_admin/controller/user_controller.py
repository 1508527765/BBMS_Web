...
async def get_system_dept_tree(
    request: Request, query_db: Session = Depends(get_db), data_scope_sql: str = Depends(GetDataScope('SysDept'))
):
...
async def add_system_user(
    ...
    if not current_user.user.admin:
        DeptService.check_dept_data_scope_services(query_db, add_user.dept_id, dept_data_scope_sql)
        RoleService.check_role_data_scope_services(
...
async def edit_system_user(
    ...
    UserService.check_user_allowed_services(edit_user)
    if not current_user.user.admin:
        UserService.check_user_data_scope_services(query_db, edit_user.user_id, user_data_scope_sql)
        DeptService.check_dept_data_scope_services(query_db, edit_user.dept_id, dept_data_scope_sql)
        RoleService.check_role_data_scope_services(
...
async def delete_system_user(
    ...
    if user_id_list:
        if current_user.user.user_id in user_id_list:
            ...
            return ResponseUtil.failure(msg='Current logged-in user cannot be deleted')
        for user_id in user_id_list:
            UserService.check_user_allowed_services(UserModel(userId=int(user_id)))
            if not current_user.user.admin:
                UserService.check_user_data_scope_services(query_db, int(user_id), data_scope_sql)
...
async def change_system_user_status(
    ...
    UserService.check_user_allowed_services(change_user)
    if not current_user.user.admin:
        UserService.check_user_data_scope_services(query_db, change_user.user_id, data_scope_sql)
    edit_user = EditUserModel(
...
async def change_system_user_profile_avatar(
    ...
    return ResponseUtil.failure(msg='Image upload error, please contact administrator')
...
async def batch_import_system_user(
    ...
    batch_import_result = UserService.batch_import_user_services(
