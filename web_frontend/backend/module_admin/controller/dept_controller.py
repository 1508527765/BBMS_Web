...
async def edit_system_dept(
    ...
    if not current_user.user.admin:
        DeptService.check_dept_data_scope_services(query_db, edit_dept.dept_id, data_scope_sql)
...
async def delete_system_dept(
    ...
    if dept_id_list:
        for dept_id in dept_id_list:
            if not current_user.user.admin:
                DeptService.check_dept_data_scope_services(query_db, int(dept_id), data_scope_sql)
...
async def query_detail_system_dept(
    ...
    if not current_user.user.admin:
        DeptService.check_dept_data_scope_services(query_db, dept_id, data_scope_sql)
