...
class RoleDao:
    ...
    @classmethod
    async def get_role_by_info(cls, db: Session, role: RoleModel):
        """
        根据角色参数获取角色信息
    ...
    @classmethod
    async def get_role_by_id(cls, db: Session, role_id: int):
        """
        根据角色id获取在用角色信息
    ...
    @classmethod
    async def get_role_detail_by_id(cls, db: Session, role_id: int):
        """
        根据role_id获取角色详细信息
        ...
        """
        query_role_info = (
            (db.execute(select(SysRole).where(SysRole.del_flag == '0', SysRole.role_id == role_id).distinct()))
            .scalars()
            .first()
        )
    ...
    @classmethod
    async def get_role_select_option_dao(cls, db: Session):
        """
        获取编辑页面对应的在用角色列表信息
    ...
    async def get_role_list(
        cls, db: Session, query_object: RolePageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        根据查询参数获取角色列表信息
    ...
    @classmethod
    async def add_role_dao(cls, db: Session, role: RoleModel):
        """
        新增角色数据库操作
    ...
    @classmethod
    async def edit_role_dao(cls, db: Session, role: dict):
        """
        编辑角色数据库操作
        ...
        :return:
        """
        db.execute(update(SysRole), [role])

    @classmethod
    async def delete_role_dao(cls, db: Session, role: RoleModel):
        """
        删除角色数据库操作
    ...
    @classmethod
    async def get_role_menu_dao(cls, db: Session, role: RoleModel):
        """
        根据角色id获取角色菜单关联列表信息
    ...
    @classmethod
    async def add_role_menu_dao(cls, db: Session, role_menu: RoleMenuModel):
        """
        新增角色菜单关联信息数据库操作
    ...
    @classmethod
    async def delete_role_menu_dao(cls, db: Session, role_menu: RoleMenuModel):
        """
        删除角色菜单关联信息数据库操作
        ...
        :return:
        """
        db.execute(delete(SysRoleMenu).where(SysRoleMenu.role_id.in_([role_menu.role_id])))

    @classmethod
    async def get_role_dept_dao(cls, db: Session, role: RoleModel):
        """
        根据角色id获取角色部门关联列表信息
    ...
    @classmethod
    async def add_role_dept_dao(cls, db: Session, role_dept: RoleDeptModel):
        """
        新增角色部门关联信息数据库操作
    ...
    @classmethod
    async def delete_role_dept_dao(cls, db: Session, role_dept: RoleDeptModel):
        """
        删除角色部门关联信息数据库操作
        ...
        :return:
        """
        db.execute(delete(SysRoleDept).where(SysRoleDept.role_id.in_([role_dept.role_id])))

    @classmethod
    async def count_user_role_dao(cls, db: Session, role_id: int):
        """
        根据角色id查询角色关联用户数量
        ...
        """
        user_count = (
            db.execute(select(func.count('*')).select_from(SysUserRole).where(SysUserRole.role_id == role_id))
        ).scalar()
