...
class UserDao:
    ...
    @classmethod
    async def get_user_by_name(cls, db: Session, user_name: str):
        """
        根据用户名获取用户信息
    ...
    @classmethod
    async def get_user_by_info(cls, db: Session, user: UserModel):
        """
        根据用户参数获取用户信息
    ...
    async def get_user_by_id(cls, db: Session, user_id: int):
        ...
        if 1 in role_id_list:
            query_user_menu_info = (
                (db.execute(select(SysMenu).where(SysMenu.status == '0').distinct())).scalars().all()
            )
        else:
            query_user_menu_info = (
                (
    ...
    def get_user_detail_by_id(cls, db: Session, user_id: int):
        ...
        """
        query_user_basic_info = (
            (db.execute(select(SysUser).where(SysUser.del_flag == '0', SysUser.user_id == user_id).distinct()))
            .scalars()
            .first()
        )
        query_user_dept_info = (
    ...
    async def get_user_list(
        cls, db: Session, query_object: UserPageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        根据查询参数获取用户列表信息
    ...
    @classmethod
    async def add_user_dao(cls, db: Session, user: UserModel):
        """
        新增用户数据库操作
    ...
    @classmethod
    async def edit_user_dao(cls, db: Session, user: dict):
        """
        编辑用户数据库操作
        ...
        :return: 编辑校验结果
        """
        db.execute(update(SysUser), [user])

    @classmethod
    async def delete_user_dao(cls, db: Session, user: UserModel):
        """
        删除用户数据库操作
    ...
    @classmethod
    async def get_user_role_allocated_list_by_user_id(cls, db: Session, query_object: UserRoleQueryModel):
        """
    ...
    async def get_user_role_allocated_list_by_role_id(
        cls, db: Session, query_object: UserRolePageQueryModel, data_scope_sql: str, is_page: bool = False
    ):
        """
        根据角色id获取已分配的用户列表信息
        ...
        allocated_user_list = PageUtil.paginate(db, query, query_object.page_num, query_object.page_size, is_page)
    ...
    async def get_user_role_unallocated_list_by_role_id(
        ...
        unallocated_user_list = PageUtil.paginate(
    ...
    @classmethod
    async def add_user_role_dao(cls, db: Session, user_role: UserRoleModel):
        """
        新增用户角色关联信息数据库操作
    ...
    @classmethod
    async def delete_user_role_dao(cls, db: Session, user_role: UserRoleModel):
        """
        删除用户角色关联信息数据库操作
        ...
        :return:
        """
        db.execute(delete(SysUserRole).where(SysUserRole.user_id.in_([user_role.user_id])))

    @classmethod
    async def delete_user_role_by_user_and_role_dao(cls, db: Session, user_role: UserRoleModel):
        """
    ...
    @classmethod
    async def get_user_role_detail(cls, db: Session, user_role: UserRoleModel):
        """
    ...
    @classmethod
    async def add_user_post_dao(cls, db: Session, user_post: UserPostModel):
        """
        新增用户岗位关联信息数据库操作
    ...
    @classmethod
    async def delete_user_post_dao(cls, db: Session, user_post: UserPostModel):
        """
        删除用户岗位关联信息数据库操作
        ...
        :return:
        """
        db.execute(delete(SysUserPost).where(SysUserPost.user_id.in_([user_post.user_id])))

    @classmethod
    async def get_user_dept_info(cls, db: Session, dept_id: int):
        dept_basic_info = (
            (
                db.execute(
