...
class ConfigDao:
    ...
    @classmethod
    async def get_config_detail_by_id(cls, db: Session, config_id: int):
        """
        根据参数配置id获取参数配置详细信息
        ...
        :return: 参数配置信息对象
        """
        config_info = (db.execute(select(SysConfig).where(SysConfig.config_id == config_id))).scalars().first()

        return config_info

    @classmethod
    async def get_config_detail_by_info(cls, db: Session, config: ConfigModel):
        """
        根据参数配置参数获取参数配置信息
    ...
    @classmethod
    async def get_config_list(cls, db: Session, query_object: ConfigPageQueryModel, is_page: bool = False):
        """
        根据查询参数获取参数配置列表信息
    ...
    @classmethod
    async def add_config_dao(cls, db: Session, config: ConfigModel):
        """
        新增参数配置数据库操作
    ...
    @classmethod
    async def edit_config_dao(cls, db: Session, config: dict):
        """
        编辑参数配置数据库操作
        ...
        :return:
        """
        db.execute(update(SysConfig), [config])

    @classmethod
    async def delete_config_dao(cls, db: Session, config: ConfigModel):
        """
        删除参数配置数据库操作
        ...
        :return:
        """
        db.execute(delete(SysConfig).where(SysConfig.config_id.in_([config.config_id])))
