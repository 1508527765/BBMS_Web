...
class DeptDao:
    ...
    @classmethod
    async def get_dept_by_id(cls, db: Session, dept_id: int):
        """
        根据部门id获取在用部门信息
        ...
        :return: 在用部门信息对象
        """
        dept_info = (db.execute(select(SysDept).where(SysDept.dept_id == dept_id))).scalars().first()

        return dept_info

    @classmethod
    async def get_dept_detail_by_id(cls, db: Session, dept_id: int):
        """
        根据部门id获取部门详细信息
        ...
        """
        dept_info = (
            (db.execute(select(SysDept).where(SysDept.dept_id == dept_id, SysDept.del_flag == '0')))
            .scalars()
            .first()
        )
    ...
    @classmethod
    async def get_dept_detail_by_info(cls, db: Session, dept: DeptModel):
        """
        根据部门参数获取部门信息
    ...
    @classmethod
    async def get_dept_info_for_edit_option(cls, db: Session, dept_info: DeptModel, data_scope_sql: str):
        """
        获取部门编辑对应的在用部门列表信息
    ...
    @classmethod
    async def get_children_dept_dao(cls, db: Session, dept_id: int):
        """
        ...
        """
        dept_result = (
            (db.execute(select(SysDept).where(func.find_in_set(dept_id, SysDept.ancestors)))).scalars().all()
        )

        return dept_result

    @classmethod
    async def get_dept_list_for_tree(cls, db: Session, dept_info: DeptModel, data_scope_sql: str):
        """
        获取所有在用部门列表信息
    ...
    @classmethod
    async def get_dept_list(cls, db: Session, page_object: DeptModel, data_scope_sql: str):
        """
        根据查询参数获取部门列表信息
    ...
    @classmethod
    async def add_dept_dao(cls, db: Session, dept: DeptModel):
        """
        新增部门数据库操作
    ...
    @classmethod
    async def edit_dept_dao(cls, db: Session, dept: dict):
        """
        编辑部门数据库操作
        ...
        :return: 编辑校验结果
        """
        db.execute(update(SysDept), [dept])

    @classmethod
    async def update_dept_children_dao(cls, db: Session, update_dept: List):
        """
        更新子部门信息
    ...
    @classmethod
    async def update_dept_status_normal_dao(cls, db: Session, dept_id_list: List):
        """
        批量更新部门状态为正常
        ...
        :return:
        """
        db.execute(update(SysDept).where(SysDept.dept_id.in_(dept_id_list)).values(status='0'))

    @classmethod
    async def delete_dept_dao(cls, db: Session, dept: DeptModel):
        """
        删除部门数据库操作
    ...
    @classmethod
    async def count_normal_children_dept_dao(cls, db: Session, dept_id: int):
        """
    ...
    @classmethod
    async def count_children_dept_dao(cls, db: Session, dept_id: int):
        """
    ...
    @classmethod
    async def count_dept_user_dao(cls, db: Session, dept_id: int):
        """
        根据部门id查询查询部门下的用户数量
