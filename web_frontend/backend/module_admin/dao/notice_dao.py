...
class NoticeDao:
    ...
    @classmethod
    async def get_notice_detail_by_id(cls, db: Session, notice_id: int):
        """
        根据通知公告id获取通知公告详细信息
        ...
        :return: 通知公告信息对象
        """
        notice_info = (db.execute(select(SysNotice).where(SysNotice.notice_id == notice_id))).scalars().first()

        return notice_info

    @classmethod
    async def get_notice_detail_by_info(cls, db: Session, notice: NoticeModel):
        """
        根据通知公告参数获取通知公告信息
    ...
    @classmethod
    async def get_notice_list(cls, db: Session, query_object: NoticePageQueryModel, is_page: bool = False):
        """
        根据查询参数获取通知公告列表信息
    ...
    @classmethod
    async def add_notice_dao(cls, db: Session, notice: NoticeModel):
        """
        新增通知公告数据库操作
    ...
    @classmethod
    async def edit_notice_dao(cls, db: Session, notice: dict):
        """
        编辑通知公告数据库操作
        ...
        :return:
        """
        db.execute(update(SysNotice), [notice])

    @classmethod
    async def delete_notice_dao(cls, db: Session, notice: NoticeModel):
        """
        删除通知公告数据库操作
        ...
        :return:
        """
        db.execute(delete(SysNotice).where(SysNotice.notice_id.in_([notice.notice_id])))
