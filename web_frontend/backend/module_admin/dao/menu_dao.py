...
class MenuDao:
    ...
    @classmethod
    async def get_menu_detail_by_id(cls, db: Session, menu_id: int):
        """
        根据菜单id获取菜单详细信息
        ...
        :return: 菜单信息对象
        """
        menu_info = (db.execute(select(SysMenu).where(SysMenu.menu_id == menu_id))).scalars().first()

        return menu_info

    @classmethod
    async def get_menu_detail_by_info(cls, db: Session, menu: MenuModel):
        """
        根据菜单参数获取菜单信息
    ...
    @classmethod
    async def get_menu_list_for_tree(cls, db: Session, user_id: int, role: list):
        """
        根据角色信息获取所有在用菜单列表信息
        ...
        if 1 in role_id_list:
            menu_query_all = (
                (db.execute(select(SysMenu).where(SysMenu.status == '0').order_by(SysMenu.order_num).distinct()))
                .scalars()
                .all()
            )
        else:
    ...
    @classmethod
    async def get_menu_list(cls, db: Session, page_object: MenuQueryModel, user_id: int, role: list):
        """
        根据查询参数获取菜单列表信息
    ...
    @classmethod
    async def add_menu_dao(cls, db: Session, menu: MenuModel):
        """
        新增菜单数据库操作
    ...
    @classmethod
    async def edit_menu_dao(cls, db: Session, menu: dict):
        """
        编辑菜单数据库操作
        ...
        :return:
        """
        db.execute(update(SysMenu), [menu])

    @classmethod
    async def delete_menu_dao(cls, db: Session, menu: MenuModel):
        """
        删除菜单数据库操作
        ...
        :return:
        """
        db.execute(delete(SysMenu).where(SysMenu.menu_id.in_([menu.menu_id])))

    @classmethod
    async def has_child_by_menu_id_dao(cls, db: Session, menu_id: int):
        """
        根据菜单id查询菜单关联子菜单的数量
        ...
        """
        menu_count = (
            db.execute(select(func.count('*')).select_from(SysMenu).where(SysMenu.parent_id == menu_id))
        ).scalar()

        return menu_count

    @classmethod
    async def check_menu_exist_role_dao(cls, db: Session, menu_id: int):
        """
        根据菜单id查询菜单关联角色数量
        ...
        """
        role_count = (
            db.execute(select(func.count('*')).select_from(SysRoleMenu).where(SysRoleMenu.menu_id == menu_id))
        ).scalar()
