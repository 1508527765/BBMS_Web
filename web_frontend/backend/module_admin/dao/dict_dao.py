...
class DictTypeDao:
    ...
    @classmethod
    async def get_dict_type_detail_by_id(cls, db: Session, dict_id: int):
        """
        根据字典类型id获取字典类型详细信息
        ...
        :return: 字典类型信息对象
        """
        dict_type_info = (db.execute(select(SysDictType).where(SysDictType.dict_id == dict_id))).scalars().first()

        return dict_type_info

    @classmethod
    async def get_dict_type_detail_by_info(cls, db: Session, dict_type: DictTypeModel):
        """
        根据字典类型参数获取字典类型信息
    ...
    @classmethod
    async def get_all_dict_type(cls, db: Session):
        """
        获取所有的字典类型信息
        ...
        :return: 字典类型信息列表对象
        """
        dict_type_info = (db.execute(select(SysDictType))).scalars().all()

        return list_format_datetime(dict_type_info)

    @classmethod
    async def get_dict_type_list(cls, db: Session, query_object: DictTypePageQueryModel, is_page: bool = False):
        """
        根据查询参数获取字典类型列表信息
    ...
    @classmethod
    async def add_dict_type_dao(cls, db: Session, dict_type: DictTypeModel):
        """
        新增字典类型数据库操作
    ...
    @classmethod
    async def edit_dict_type_dao(cls, db: Session, dict_type: dict):
        """
        编辑字典类型数据库操作
        ...
        :return:
        """
        db.execute(update(SysDictType), [dict_type])

    @classmethod
    async def delete_dict_type_dao(cls, db: Session, dict_type: DictTypeModel):
        """
        删除字典类型数据库操作
        ...
        :return:
        """
        db.execute(delete(SysDictType).where(SysDictType.dict_id.in_([dict_type.dict_id])))
...
class DictDataDao:
    ...
    @classmethod
    async def get_dict_data_detail_by_id(cls, db: Session, dict_code: int):
        """
        根据字典数据id获取字典数据详细信息
        ...
        """
        dict_data_info = (
            (db.execute(select(SysDictData).where(SysDictData.dict_code == dict_code))).scalars().first()
        )

        return dict_data_info

    @classmethod
    async def get_dict_data_detail_by_info(cls, db: Session, dict_data: DictDataModel):
        """
        根据字典数据参数获取字典数据信息
    ...
    @classmethod
    async def get_dict_data_list(cls, db: Session, query_object: DictDataPageQueryModel, is_page: bool = False):
        """
        根据查询参数获取字典数据列表信息
        ...
        dict_data_list = PageUtil.paginate(db, query, query_object.page_num, query_object.page_size, is_page)

        return dict_data_list

    @classmethod
    async def query_dict_data_list(cls, db: Session, dict_type: str):
        """
        根据查询参数获取字典数据列表信息
    ...
    @classmethod
    async def add_dict_data_dao(cls, db: Session, dict_data: DictDataModel):
        """
        新增字典数据数据库操作
    ...
    @classmethod
    async def edit_dict_data_dao(cls, db: Session, dict_data: dict):
        """
        编辑字典数据数据库操作
        ...
        :return:
        """
        db.execute(update(SysDictData), [dict_data])

    @classmethod
    async def delete_dict_data_dao(cls, db: Session, dict_data: DictDataModel):
        """
        删除字典数据数据库操作
        ...
        :return:
        """
        db.execute(delete(SysDictData).where(SysDictData.dict_code.in_([dict_data.dict_code])))

    @classmethod
    async def count_dict_data_dao(cls, db: Session, dict_type: str):
        """
        ...
        """
        dict_data_count = (
            db.execute(select(func.count('*')).select_from(SysDictData).where(SysDictData.dict_type == dict_type))
        ).scalar()
