...
def export_list2csv(list_data: List):
    """
    工具方法：将需要导出的list数据转化为对应csv的二进制数据

    :param list_data: 数据列表
    :return: 字典信息对应csv的二进制数据
    """
    df = pd.DataFrame(list_data)
    binary_data = io.BytesIO()
    # CSV 使用 utf-8-sig 防止 Excel 打开中文乱码
    df.to_csv(binary_data, index=False, encoding="utf-8-sig")
    binary_data = binary_data.getvalue()
    return binary_data


def get_excel_template(header_list: List, selector_header_list: List = None, option_list: List[dict] = None):
    """
    工具方法：将需要导出的list数据转化为对应excel的二进制数据
    ...
    # 设置选择器的预设选项
    if selector_header_list and option_list:
        options = option_list
